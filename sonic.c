...
/* 音量调整，按因子对样品进行缩放Scale the samples by the factor. */
    ...
    int numChannels)
{
    ...
    stream->oldRatePosition = 0;
    stream->newRatePosition = 0;
    stream->minPeriod = minPeriod;/*最小的基音周期*/
    stream->maxPeriod = maxPeriod;/*最大的基音周期*/
...
/* 如果需要扩大输入缓冲区Enlarge the input buffer if needed. */
...
/* 把输入样本添加到输入字节中Add the input samples to the input buffer. */
...
static void downSampleInput(
    ...
    int skip)
{
    int numSamples = stream->maxRequired/skip;/*2倍最大周期/skip**/
    ...
    int *retMaxDiff)
{
    ...
    int i;
    /*
		int minPeriod = sampleRate/SONIC_MAX_PITCH;//最小基音周期
		int maxPeriod = sampleRate/SONIC_MIN_PITCH;//最大基音周期
	*/
...
/* 查找基音周期Find the pitch period.  This is a critical step, and we may have to try
    ...
    int preferNewPeriod)
{
    ...
    if(stream->numChannels == 1 && skip == 1) {/*4k*/
	period = findPitchPeriodInRange(samples, minPeriod, maxPeriod, &minDiff, &maxDiff);
    } else {
	downSampleInput(stream, samples, skip);/*下采样提升查找速度*/
	/**获取周期*/
...
/*重叠的两个声音片段,坡道的体积,而另一个从零,并将它们添加、存储在输出结果
    ...
    short *rampUp)
{
    ...
    for(i = 0; i < numChannels; i++) {
		o = out + i;/*out叠加后输出目标***/
		u = rampUp + i;/*rampup叠加的输入*/
		d = rampDown + i;/*rampDown叠加的输入*/
